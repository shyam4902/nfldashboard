# Browser-test fixtures (hermetic data feeds)
...
That makes suite results independent of live Supabase contents, the nflverse
file, and today's network state for project data.
...
| `a.espncdn.com` (team logos) | 1x1 PNG — no network image loads, no console noise |
...
data. Full offline hermeticity would require vendoring them; that is a
deliberate non-goal of these fixtures.

## Opt-in live mode

Hermetic interception is the default test path. To exercise real Supabase and
